...
- 使用 Azure OpenAI 时，Azure 可能不会正确处理 `"detail": "original"`，从而造成点击坐标偏移。详见 [使用 Azure OpenAI 时点击坐标偏移](./faq#使用-azure-openai-时点击坐标偏移)。
...
### Muse Spark 系列 {#muse-spark}

- 常用模型供应商：[Meta Model API](https://dev.meta.ai/docs/)、[OpenRouter](https://openrouter.ai/meta/muse-spark-1.3-contributor)

<div className="model-series-table">

| 模型版本 | 常用模型名称 | `MIDSCENE_MODEL_FAMILY` | 备注 |
| --- | --- | --- | --- |
| Muse Spark 1.3 系列 | `muse-spark-1.3`、`muse-spark-1.3-contributor` | `muse-spark` | 带视觉能力的推理模型。Contributor 档更便宜，适合实验。单个 Default 模型即可覆盖 planning、insight 和 locate，无需拆分。 |

</div>

环境变量配置示例，以 OpenRouter 上的 `meta/muse-spark-1.3-contributor` 为例：

<ModelConfigTabs>
  <ModelConfigTab type="default">

```bash
MIDSCENE_MODEL_BASE_URL="https://openrouter.ai/api/v1" # OpenRouter 地址；Meta Model API 使用 https://api.meta.ai/v1
MIDSCENE_MODEL_API_KEY="......"
...
Muse 推理说明：Chat Completions 使用顶层 `reasoning_effort`（`minimal`/`low`/`medium`/`high`/`xhigh`），不支持 `none`。Midscene 在未设置或 `default` 时省略该参数，`reasoningEnabled=true` 映射为 `medium`，`reasoningEnabled=false` 映射为 `minimal`。
